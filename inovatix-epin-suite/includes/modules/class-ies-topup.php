...
class IES_Topup {
    ...
    public function __construct() {
       ...
       add_action( 'init', array( $this, 'add_endpoint' ) );
        ...
        add_shortcode( 'ies_wallet_balance', array( $this, 'render_wallet_balance_shortcode' ) );
  ...
  public function account_menu( $items ) {
        ...
        if ( is_user_logged_in() && $this->wallet ) {
            $balance = $this->wallet->get_balance( get_current_user_id() );
            $label   = sprintf( 'Bakiye Yükle (%s TL)', number_format_i18n( $balance, 2 ) );
        }

        $items['bakiye-yukle'] = $label;
    ...
    /**
     * [ies_wallet_balance] shortcode to show the current user's balance.
     */
    public function render_wallet_balance_shortcode( $atts ) {
        $atts = shortcode_atts(
            array(
                'show_label' => 'yes',
                'decimals'   => 2,
                'fallback'   => 'Giriş yapınız',
            ),
            $atts,
            'ies_wallet_balance'
        );
        ...
        $formatted = number_format_i18n( $balance, $decimals ) . ' TL';

        if ( strtolower( $atts['show_label'] ) === 'yes' ) {
            $formatted = 'Bakiye: ' . $formatted;
        }

        return '<span class="ies-wallet-balance-shortcode">' . esc_html( $formatted ) . '</span>';
    }
