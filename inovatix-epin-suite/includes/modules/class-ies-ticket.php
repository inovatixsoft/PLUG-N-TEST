...
class IES_Ticket {
    ...
    public function __construct() {
        ...
        add_action( 'wp_ajax_ies_ticket_latest', array( $this, 'ajax_latest_ticket' ) );
    }
...
" . admin_url( "admin.php?page=ies-tickets&ticket_id={$ticket_id}" );
    ...
    /* ============================================================
     *  ADMIN NOTIFICATION AUDIO
     * ============================================================ */

    /**
     * Enqueue admin script to poll new tickets and play a sound.
     */
    public function enqueue_admin_assets( $hook ) {
        ...
        $is_ticket_page = isset( $_GET['page'] ) && $_GET['page'] === 'ies-tickets';
        ...
        wp_enqueue_script(
            'ies-admin-ticket',
            IES_URL . 'assets/js/admin-ticket.js',
            array( 'jquery' ),
            IES_VERSION,
            true
        );

        $latest = $this->get_latest_ticket_summary();
        ...
        wp_localize_script(
            ...
            array(
                ...
                'nonce'    => wp_create_nonce( 'ies_ticket_latest' ),
                'latestId' => $latest ? intval( $latest['id'] ) : 0,
                'interval' => apply_filters( 'ies_ticket_check_interval', 30000 ),
                'audioSrc' => 'data:audio/wav;base64,UklGRoQAAABXQVZFZm10IBAAAAABAAEAIlYAAESsAAACABAAZGF0YYQAAAAA',
            )
        );
    }

    /**
     * Return the latest ticket id and timestamp.
     */
    protected function get_latest_ticket_summary() {
        global $wpdb;

        $row = $wpdb->get_row(
            "SELECT id, created_at FROM {$this->table_tickets} ORDER BY id DESC LIMIT 1",
            ARRAY_A
        );
    ...
    /**
     * Ajax endpoint used by admin polling script.
     */
    public function ajax_latest_ticket() {
        if ( ! current_user_can( 'manage_woocommerce' ) ) {
            wp_send_json_error();
        }

        check_ajax_referer( 'ies_ticket_latest' );
